...
```js
import { init, registerComponent } from '/path/to/het.js';
...
- Request-driven content loads dispatch `het:sync` automatically, and other DOM-mutating integrations can dispatch the same event manually so components reconcile external DOM changes back into signals.
