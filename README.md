...
`init` accepts an optional config object shared by both **components** and **requests**:

- `onError`: callback invoked when an internal error is caught (default is to rethrow)
- `busyClass` (requests): CSS class added to the target pane while a request is in flight (default: `het-busy`)
- `headContentSelectors` (requests): array of selectors for `<head>` elements to sync from responses (title/meta/link/script by default)

```js
init({
  onError: (error) => {
    console.error("HET caught error", error);
    // Custom reporting/logging here
  },
